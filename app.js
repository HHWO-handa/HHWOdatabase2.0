...
const DEMO_INSTRUMENTS = [
  { id:1, instnumber:1, qr:'QR-001', instcode:'CL-001', name:'Bb クラリネット', maker:'Buffet Crampon', model:'R13', purchase_date:'2020-04-01', owner:'田中 花子', equipcode:'EQ-001', image_url:null, notes:'リードは3番を使用。' },
  { id:2, instnumber:2, qr:'QR-002', instcode:'SAX-001', name:'テナーサックス', maker:'Yamaha', model:'YTS-82Z', purchase_date:'2019-09-15', owner:'鈴木 一郎', equipcode:'EQ-002', image_url:null, notes:'定期メンテナンス済み（2023年10月）' },
  { id:3, instnumber:3, qr:'QR-003', instcode:'TP-001', name:'トランペット', maker:'Bach', model:'Stradivarius 37', purchase_date:'2021-03-20', owner:'佐藤 次郎', equipcode:'EQ-003', image_url:null, notes:'バルブ不調のため修理依頼中。' },
];
const DEMO_SCORES = [
...
const DEMO_REPAIRS = [
  { id:1, repair_id:'RPR-A1B2C3D4', instnumber:3, qr:'QR-003', instcode:'TP-001', status:'違和感による修理', repair_content:'バルブの動きが重く感じる', repair_photo:null, repair_date:'2024-01-15' },
  { id:2, repair_id:'RPR-E5F6G7H8', instnumber:2, qr:'QR-002', instcode:'SAX-001', status:'定期点検', repair_content:'年次点検', repair_photo:null, repair_date:'2023-10-20' },
];
    ...
    <table class="data-table">
      <thead>
        <tr>
          ...
          <th>楽器No.</th>
          <th>状態</th>
          <th class="hide-mobile">修理日</th>
          <th></th>
        </tr>
      </thead>
      <tbody>
        ${filtered.map(r => {
          // instruments から紐づけて画像取得
          const inst = allInstruments.find(i =>
            i.instnumber === r.instnumber || i.qr === r.qr
          );
...
function openRepairDetail(id) {
  ...
  if (!item) return;
  const inst = allInstruments.find(i => i.instnumber === item.instnumber || i.qr === item.qr);
  const photoHtml = item.repair_photo
    ? `<img src="${esc(item.repair_photo)}" class="detail-image" onerror="this.style.display='none'">`
    : '';
  const noteHtml = item.repair_content
    ? `<div class="detail-note">${esc(item.repair_content)}</div>`
    : '<span style="color:var(--text-muted)">なし</span>';
...
function renderAdminPanel() {
  ...
  const rows = filtered.map(item => {
    const label = type === 'instrument' ? item.name
                : type === 'score' ? item.jatitle
                : item.repair_id;
    const sub = type === 'instrument' ? String(item.instnumber||'')
              : type === 'score' ? String(item.scorenumber||'')
              : item.status||'';
    return `
      <tr>
  ...
  document.getElementById('adminContent').innerHTML = `
    ...
    <div class="admin-search-wrap">
      <input type="text" class="admin-search-input" placeholder="管理者画面内を検索..." value="${esc(adminSearchQuery)}"
        oninput="adminSearchQuery=this.value; renderAdminPanel()">
    ...
    <div style="overflow-x:auto">
      <table class="admin-table">
        <thead>
          <tr>
            <th>${type === 'instrument' ? 'No.' : type === 'score' ? 'No.' : 'ID'}</th>
      ...
      <div class="image-upload-area">
        ...
        <div class="image-upload-btns">
          <label class="upload-file-btn">
            📁 ファイルを選択してアップロード
            <input type="file" id="f_image_file" accept="image/*" style="display:none" onchange="handleImageSelect(event,'instrument')">
          </label>
        </div>
        <input type="text" class="form-input" id="f_image_url" value="${v('image_url')}"
          placeholder="またはURLを直接入力" oninput="previewImageUrl(this.value,'imagePreviewWrap')" style="margin-top:8px">
...
function repairForm(item) {
  ...
  const instOptions = allInstruments.map(i =>
    `<option value="${i.instnumber}" data-qr="${esc(i.qr)}" data-instcode="${esc(i.instcode||'')}"
      ${item?.instnumber === i.instnumber ? 'selected' : ''}>
      #${i.instnumber} ${esc(i.name)}
    </option>`
  ).join('');
  ...
  return `
    ...
    <div class="form-group">
      ...
      <select class="form-select" id="f_instnumber" onchange="onInstSelect(this)">
    ...
    <div class="form-group">
      ...
      <div class="image-upload-area">
        ...
        <div class="image-upload-btns">
          <label class="upload-file-btn">
            📷 写真を選択してアップロード
            <input type="file" id="f_repair_photo_file" accept="image/*" style="display:none" onchange="handleImageSelect(event,'repair')">
          </label>
        </div>
        <input type="text" class="form-input" id="f_repair_photo" value="${v('repair_photo')}"
          placeholder="またはURLを直接入力" oninput="previewImageUrl(this.value,'repairPreviewWrap')" style="margin-top:8px">
        <div id="uploadStatus" class="upload-status"></div>
      </div>
    </div>
...
async function saveInstrument() {
  ...
  const data = {
    instnumber, qr: qrVal || `QR-${String(instnumber).padStart(3,'0')}`,
