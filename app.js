...
function switchPage(page) {
  ...
  // 楽譜フィルターバー表示制御
  const isScorePage = page === 'scores' || page === 'baseball';
  document.getElementById('scoreFilterBar').style.display = isScorePage ? 'block' : 'none';
  if (!isScorePage) resetScoreFilter();
...
function renderPage() {
  ...
  switch(currentPage) {
    ...
    case 'scores':      renderScoresTable(allScores.filter(s => !s.is_baseball), query, filterCat, filterVal); break;
    case 'repair':      renderRepairsTable(allRepairs, query); break;
    case 'baseball':    renderScoresTable(allScores.filter(s => s.is_baseball), query, filterCat, filterVal); break;
  }
}
...
// 楽譜テーブル描画
// ============================================================
function renderScoresTable(data, query = '', filterCat = '', filterVal = '') {
  // テキスト検索：全フィールド対象
  let filtered = query ? data.filter(s =>
    ...
    (s.instrumentation||'').toLowerCase().includes(query) ||
    (s.notes||'').toLowerCase().includes(query) ||
    String(s.scorenumber||'').includes(query)
  ) : data;

  // プルダウンフィルター
  if (filterCat && filterVal) {
    filtered = filtered.filter(s => {
      if (filterCat === 'hiragana') return (s.hiragana||'') === filterVal;
      if (filterCat === 'alphabet') return (s.alphabet||'').toUpperCase() === filterVal.toUpperCase();
      if (filterCat === 'instrumentation') return (s.instrumentation||'') === filterVal;
      return true;
    });
  }
...
async function upsertData(table, id, data, localArray) {
  if (!db || SUPABASE_URL === 'YOUR_SUPABASE_URL') {
    ...
    if (error) {
      console.error(error);
      if (error.code === '23505' && error.details && error.details.includes('Key (id)')) {
        alert('保存失敗：IDの重複エラーです。

Supabase の SQL Editor で以下3行を実行してください:

SELECT setval(\'scores_id_seq\', (SELECT MAX(id) FROM scores));
SELECT setval(\'instruments_id_seq\', (SELECT MAX(id) FROM instruments));
SELECT setval(\'repairs_id_seq\', (SELECT MAX(id) FROM repairs));');
      } else {
        alert('保存に失敗しました:
' + error.message);
      }
...
// 楽譜フィルター（2層プルダウン）
// ============================================================
const HIRAGANA_LIST = ['あ','い','う','え','お','か','き','く','け','こ','さ','し','す','せ','そ','た','ち','つ','て','と','な','に','ぬ','ね','の','は','ひ','ふ','へ','ほ','ま','み','む','め','も','や','ゆ','よ','ら','り','る','れ','ろ','わ','を','ん'];
const ALPHABET_LIST = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.split('');
const INSTRUMENTATION_LIST = ['吹奏楽','木管','金管','打楽器','フルート','クラリネット','サックス','トランペット','トロンボーン','ホルン','バリ・チューバ','フレックス'];

function onFilterCategoryChange() {
  const cat = document.getElementById('filterCategory').value;
  const valGroup = document.getElementById('filterValueGroup');
  ...
  if (!cat) {
    ...
    valSelect.innerHTML = '<option value="">すべて</option>';
    renderPage();
    return;
  }
  ...
  if (cat === 'hiragana') {
    options += HIRAGANA_LIST.map(h => `<option value="${h}">${h}</option>`).join('');
  } else if (cat === 'alphabet') {
    options += ALPHABET_LIST.map(a => `<option value="${a}">${a}</option>`).join('');
  } else if (cat === 'instrumentation') {
    options += INSTRUMENTATION_LIST.map(i => `<option value="${i}">${i}</option>`).join('');
  }
...
function resetScoreFilter() {
  ...
  const resetBtn = document.getElementById('filterResetBtn');
  if (cat) cat.value = '';
  if (valSelect) valSelect.innerHTML = '<option value="">すべて</option>';
